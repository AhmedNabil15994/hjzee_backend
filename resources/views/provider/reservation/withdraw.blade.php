                            ...
                            <div class="col-12 overflow-hidden">
                                <div class="transaction-card mt-2 mb-2 wow fadeInDown">
                                    <div class="d-flex align-items-center">
                                        <a href="{{ route('provider.reservationDetails',[$reservation]) }}"><img class="img-trans" src="{{ $reservation->user->image??asset('/storage/images/users/default.png')}}"></a>
                                        <div class="d-flex flex-column">
                                            <h6 class="name-trans">{{$reservation->user->name??$reservation->name}}</h6>
                                            <h6 class="offer-trans"> {{__('provider.phone')}} :<span>0{{$reservation->user->phone??$reservation->phone}}</span></h6>
                                            <h6 class="offer-trans">{{__('provider.order_num')}} :<span>{{  $reservation->order_num}}</span></h6>
                                            <h6 class="offer-trans">{{__('provider.paid_price')}} :<span>{{  $reservation->paid_price}}</span></h6>
                                            <h6 class="offer-trans">{{__('provider.final_price')}} :<span>{{  $reservation->final_price}}</span></h6>
                                        </div>
                                    </div>
                                    ...
                                    {{-- <a class="edit-profil" href="{{ route('provider.editReservation',[$reservation]) }}"> 
        ...
        <script type="text/javascript">
                ...
                $(document).on('keyup', '#srch', function(e){
                    ...
                    if (search == "") {
                        ...
                        $.post("<?=route('provider.searchWithdrawReservation');?>",{phone:search,"_token": "{{ csrf_token() }}"},function(data){
