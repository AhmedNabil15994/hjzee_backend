...
//  use NotifyTrait;
  ...
  public function addToWalletOnlineSuccess($transactionable,$amount,$onlinePayResponse) {
  ...
  public function adminAddtoUserWallet($transactionable, $amount,$type = 'ReceiveBalanceFromAdmin',Order | null $forable = null) {
    # transfer from supert admin to $transactionable wallet
    $this->cutFromWallet($this->superAdmin, $amount, $type,$forable);
    $this->addToWallet($transactionable, $amount, $type,$forable);
  }

  public function adminCutFromUserWallet($transactionable, $amount ,$type = 'CutBalanceByadmin',Order | Settlement | null $forable = null) {
    # transfer from supert admin to $transactionable wallet
    $this->cutFromWallet($transactionable, $amount, $type,$forable);
    $this->addToWallet($this->superAdmin, $amount, $type,$forable);
  }
  
  public function onlineOrderPaySuccess($order,$onlinePayResponse) {
    /**
     * with success pay
  ...
  public function cutFromWallet(
    ...
    $transactionable->transactions()->create([
      'dept'         => abs($amount),
      'type'         => $type,
      'forable_id'   => ($forable->id)??null,
      'forable_type' => ($forable)? \get_class($forable) : null,
    ]);
  ...
  public function addToWallet(
    ...
    $transactionable->transactions()->create([
      ...
      'forable_id'   => ($forable->id)??null,
      'forable_type' => ($forable)? \get_class($forable) : null,
      'pay_data'     => $this->onlinePayResponse? $this->onlinePayResponse : null,
    ]);
