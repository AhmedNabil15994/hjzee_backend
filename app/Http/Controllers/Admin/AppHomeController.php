...
class AppHomeController extends Controller
{
    ...
    public function getRecordsByType(Request $request){
        if($request->type == 'services'){
            $models  = Service::Active()->orderBy('name','ASC')->get();
        }elseif($request->type == 'places'){
            $models  = Place::Active()->orderBy('name','ASC')->get();
        }elseif($request->type == 'categories'){
            $models  = Category::Active()->where('parent_id',null)->orderBy('sort','ASC')->get();
    ...
    public function edit($id)
    {
        ...
        if($apphome->type == 'services'){
            $records  = Service::Active()->orderBy('name','ASC')->get();
        }elseif($apphome->type == 'places'){
            $records  = Place::Active()->orderBy('name','ASC')->get();
        }elseif($apphome->type == 'categories'){
            $records  = Category::Active()->where('parent_id',null)->orderBy('sort','ASC')->get();
